...
class OAuth2Method(AuthenticationMethod):
    ...
    def __init__(self, u_id, client_id, client_secret, access_token, refresh_token, expires_access, redirect_url):
        super(OAuth2Method, self).__init__()
        self.client_id = client_id
        self.client_secret = client_secret
        self.authorized_client = None
        self.code = None
        self.access_token = access_token  # need in all request
        ...
        self.u_id = u_id  # 目前自定义的id
        self.expires_access = expires_access  # access_token的失效日期时间（长度为60min）
    ...
    def setActionToken(self):
        ...
        self.expires_action = current + (30 - 5) * 60
    ...
    def setAccessToken(self):
        ...
        if 'access_token' not in response or 'refresh_token' not in response:
            ...
            self.authFromAccessToken(access_token=response['access_token'], \
    ...
    def is_access_token_expires(self):
        ...
        return not self.access_token or time.time() >= float(self.expires_access)

    def is_action_token_expires(self):
        return not self.action_token or time.time() >= float(self.expires_action)
    ...
    def authFromAccessToken(self, access_token, refresh_token=None, expires_in=60 * 60):
        ...
        if refresh_token is not None :
            self.refresh_token = refresh_token
        current = int(time.time())
        self.expires_access = current + expires_in - 5 * 60
        djangodb.create_or_update_google_auth(u_id=self.u_id, \
                                              access_token=self.access_token, \
                                              refresh_token=self.refresh_token, \
                                              expires_in=self.expires_access)
